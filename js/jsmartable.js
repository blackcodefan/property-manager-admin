jQuery.fn.jsmartable = function(options){
            ...
            rows.each(function(index) {
                ...
                if(typeof formated == "undefined" && totalElement > 0) { //Create jsmartable
                    ...
                    if((typeof expanded == "undefined" || expanded == false) && (typeof allExpanded == "undefined" || allExpanded == false)) {
                        jQuery("#jsmartable-row-"+index).hide();
                        buttonIcon = settings.iconPlus;
                    }
                    
                    //Add button collapse
                    jQuery("#jsmartable-row-"+index).prev().find('td:visible').first().prepend(`<a href='javascript:void(0);' class="mr-2 jsmartable-collapse" data-opened="true">${buttonIcon}</a>`);
                }
                else if(formated && totalElement > 0) { //update jsmartable

                    jQuery('#jsmartable-row-'+index+' > td').html(html);
                }
                else if(totalElement <= 0) { //Remove jsmartable

                    row.removeData('formated');

                    jQuery("#jsmartable-row-"+index).remove();

                    //remove button collapse
                    jQuery(".jsmartable-collapse").remove();
                }
            });
        });
    }
    ...
    jQuery('body').on('click', '.jsmartable-collapse', function() {
